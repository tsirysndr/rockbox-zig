...
#endif
      if (lang_is_rtl()) {
        splash_progress(ret, tagcache_get_max_commit_step(), "[%d/%d] %s", ret,
                        tagcache_get_max_commit_step(),
                        str(LANG_TAGCACHE_INIT));
      } else {
        splash_progress(ret, tagcache_get_max_commit_step(), "%s [%d/%d]",
...
static void init(void) {
  ...
  audio_init();
  talk_announce_voice_invalid(); /* notify user w/ voice prompt if voice file
                                    invalid */
  settings_apply_skins();

/* do USB last so prompt (if enabled) can work correctly if USB was inserted
 * with device off, also doesn't hurt that it will display the nice pretty
 * backdrop this way too. */
#ifndef USB_NONE
  usb_init();
  usb_start_monitoring();
#endif
...
#if (CONFIG_STORAGE & STORAGE_MMC)
      if (!mmc_touched() || (mmc_remove_request() == SYS_HOTSWAP_EXTRACTED))
#endif
      {
...
#if defined(BUTTON_REC) || (CONFIG_KEYPAD == GIGABEAT_PAD) ||                  \
    (CONFIG_KEYPAD == IPOD_4G_PAD) || (CONFIG_KEYPAD == IRIVER_H10_PAD)
