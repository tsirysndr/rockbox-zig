...
pub fn pair_verify(
    ...
    let m1 = tlv8::encode(&[
        ...
        Tlv8Item {
            typ: PUBLIC_KEY,
            value: ephemeral_pub.as_bytes().to_vec(),
        },
    ]);
    ...
    if let Some(server_id) = tlv8::find(&items_inner, IDENTIFIER) {
        if let Some(sig_bytes) = tlv8::find(&items_inner, SIGNATURE) {
            if let (Some(server_ltpk), _) = load_server_ltpk(server_id) {
                let msg: Vec<u8> = [
                    server_pub.as_bytes().as_ref(),
                    ephemeral_pub.as_bytes().as_ref(),
                ]
                .concat();
    ...
    let m3_inner = tlv8::encode(&[
        ...
        Tlv8Item {
            typ: SIGNATURE,
            value: client_sig.to_bytes().to_vec(),
        },
    ]);
    ...
    // Derive session key
    let session_key = derive_key(shared.as_bytes(), b"MediaRemote-Salt", b"MediaRemote-Key");
