...
enum codec_status codec_run(void)
{
    ...
    while (!endofstream)
    {
        ...
        if (action == CODEC_ACTION_SEEK_TIME)
        {
            ...
            newpos = (((uint64_t)avgbytespersec * param) / (1000LL * 18 * channels)) * (18 * channels);
        ...
        if (bufoff > ci->filesize - channels * 18)
            break; /* End of stream */
            ...
            ((!looping) || bufoff <= end_adr - 18 * channels))
        {
            ...
            {
                fade_count--;
                for (i = 0; i < (channels == 1 ? 32 : 64); i++)
                    samples[sampleswritten - i - 1] =
