...
static bool flac_init(FLACContext *fc, int first_frame_offset)
{
    ...
    while (!endofmetadata)
    {
        ...
        {
            if (ci->read_filebuf(buf, blocklength) < blocklength)
                return false;
...
static bool flac_seek(FLACContext *fc, uint32_t target_sample)
{
    ...
    while (1)
    {
         ...
         */
        {
            ...
            for (unparseable_count = 0; !got_a_frame && unparseable_count < 30; unparseable_count++)
            {
                if (frame_sync(fc))
                    got_a_frame = true;
            }
...
enum codec_status codec_run(void)
{
    ...
    ci->configure(DSP_SET_STEREO_MODE, fc.channels == 1 ? STEREO_MONO : STEREO_NONINTERLEAVED);
    ...
    else if (!flac_seek(&fc, (uint32_t)((uint64_t)elapsedtime * ci->id3->frequency / 1000)))
    {
        elapsedtime = 0;
    }
    ...
    while (bytesleft)
    {
        ...
        {
            if (flac_seek(&fc, (uint32_t)(((uint64_t)param * ci->id3->frequency) / 1000)))
            {
