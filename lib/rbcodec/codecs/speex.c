            ...
            if (ret > 0)
            {
                if (lastgranule != -1)
                {
                    if (avgpagelen < 0)
                        ...
                        avgpagelen = ((spx_ogg_page_granulepos(og) - lastgranule) + avgpagelen) / 2;
                }
                ...
                { /*if (spx_ogg_page_granulepos(&og)<wantedpos)*/
                    /*too early*/
                    offset = ret;
                    continue;
                }
                                   ...
                                   spx_int64_t headerssize)
{
    ...
    *curbyteoffset = ci->curpos;
    ...
    if (abs(pos - curpos) > 10000 && headerssize > 0 && curoffset - headerssize > 10000)
    {
        ...
        if (offset < 0)
        { /* could not find new page,use old offset */
                ...
                else
                {
                    ...
                    for (j = 0; j != nframes; j++)
                    {
                        ...
                        if (frame_size > 0)
                        {
                            ...
                            ci->set_elapsed((samplerate == 0) ? 0 : cur_granule * 1000LL / samplerate);
                        }
                    }
                }
                packet_count++;
