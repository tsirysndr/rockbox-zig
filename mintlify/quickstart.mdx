...
<Tabs>
  ...
  <Tab title="Docker">
    ...
    Then open the web UI at [http://localhost:6062](http://localhost:6062) and
    start playing — no config file needed.

    <Note>
      Ports `1704` and `1705` are the Snapcast client and control ports.
      Port `1780` is the Snapcast HTTP management interface.
