...
impl Status {
    ...
    pub fn get_htstamp(&self) -> timespec {
        ...
        // Guard against libasound builds where snd_pcm_status_get_htstamp is a
        // static inline (not exported) — its PLT entry resolves to 0 at runtime,
        // causing SIGSEGV at 0x00000000. cpal treats a zeroed result as "no
        // hardware timestamp" and falls back to Instant::now(), which is safe.
        let fn_addr = alsa::snd_pcm_status_get_htstamp as usize;
        if fn_addr != 0 {
    ...
    pub fn get_trigger_htstamp(&self) -> timespec {
        let mut h = timespec {tv_sec: 0, tv_nsec: 0};
        let fn_addr = alsa::snd_pcm_status_get_trigger_htstamp as usize;
        if fn_addr != 0 {
    ...
    pub fn get_audio_htstamp(&self) -> timespec {
        let mut h = timespec {tv_sec: 0, tv_nsec: 0};
        let fn_addr = alsa::snd_pcm_status_get_audio_htstamp as usize;
        if fn_addr != 0 {
