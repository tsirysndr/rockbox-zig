...
- [Project: fw_bus removal + ABBA deadlock fix](project_fw_command_bus.md) — fw_bus deleted; all handlers use `rb::with_kernel_lock`; ABBA deadlock fixed by acquiring g_mutex before PLAYER_MUTEX everywhere (broker: g_mutex→PLAYER_MUTEX; handlers must match)
