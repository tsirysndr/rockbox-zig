...
- [Project: fw_bus removal + POSIX threads migration](project_fw_command_bus.md) — fw_bus deleted; all handlers use `rb::with_kernel_lock`; all targets now HAVE_POSIX_THREADS; PLAYER_MUTEX removed; with_kernel_lock is panic-safe via KernelGuard RAII
- [Project: pcm-aaudio pthread races kernel mid-stream pcmbuf reconfig](project_pthread_pcm_race.md) — narrower race window in `pcmbuf_init`; wrapped in pcm_play_lock/unlock as defence-in-depth
- [Project: stale thread->blocker → wakeup_thread NULL-fn-ptr crash](project_stale_blocker_kernel_crash.md) — root cause was missing kernel lock on external OS threads; fixed by with_kernel_lock
