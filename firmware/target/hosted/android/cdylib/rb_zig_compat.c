 ...
 *
 * crates/sys/src/lib.rs declares 18 `extern "C" fn rb_*` symbols
 * (lib.rs:1068-1268) that are defined by Zig wrappers in
 * zig/src/rockbox/*.zig. The Android cdylib build doesn't run Zig, so we
 * provide bit-identical C implementations here.
 *
 * Each function is a near-mechanical port of its Zig counterpart. When the
 * Zig source changes, mirror the change here.
 */
...
/* ── Tree wrappers — port of zig/src/rockbox/tree.zig ─────────────────── */
...
/* ── Metadata wrapper — port of zig/src/rockbox/metadata.zig ──────────── */
