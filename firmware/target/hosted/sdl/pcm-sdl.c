...
extern int process_pcm_buffer(Uint8 *data, size_t size);
...
static void sdl_audio_callback(struct pcm_udata *udata, Uint8 *stream,
                               int len) {
...
const void *pcm_rec_dma_get_peak_buffer(void) { return NULL; }
...
#ifdef HAVE_SPDIF_IN
unsigned long spdif_measure_frequency(void) { return 0; }
#endif
...
void pcm_play_dma_init(void) {
  ...
  wanted_spec.callback = (void(SDLCALL *)(void *userdata, Uint8 *stream,
                                          int len))sdl_audio_callback;
